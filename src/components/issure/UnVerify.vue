    ...
    <div>
        ...
        <div>
            <el-table border :data="tableData" style="width: 100%"
                ...
                <el-table-column prop="title" align="center" label="标题" width="500"
                                 show-overflow-tooltip></el-table-column>
        ...
        <div>
            <el-dialog center title="详情" :visible.sync="dialogFormVisible">
                ...
                <div slot="footer" class="dialog-footer">
                    <el-button type="danger" round @click="dialogTableVisibleReason = true">不通过</el-button>
                    <el-button type="success" round @click="pass()">通过</el-button>
                </div>
            </el-dialog>

            <el-dialog title="拒绝理由" :visible.sync="dialogTableVisibleReason" center>
                <el-input style="font-size: 20px" type="textarea" v-model="reason"></el-input>
                <div slot="footer" class="dialog-footer">
                    <el-button type="danger" @click="dialogTableVisibleReason = false">取消</el-button>
                    <el-button type="primary" @click="unPass()">确认</el-button>
                </div>
            </el-dialog>
        </div>
    </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                dialogTableVisibleReason: false//控制理由窗口显示或者隐藏
            }
        },
        created() {
        ...
        methods: {
            ...
            handleCurrentChange(currentPage) {
                ...
                if (loginname.endsWith("_op")) {
                    postRequest("/issure/issure/search/0/" + id, this.pagination).then((res) => {
                    ...
                    //否则查询所有未审核投票
                    postRequest("/issure/issure/search/0", this.pagination).then((res) => {
                        this.pagination.total = res.data.total;
                        this.tableData = res.data.rows;
                    }, (err) => {
                    });
            ...
            unPass() {
                ...
                this.verifyData.reason = this.reason;
